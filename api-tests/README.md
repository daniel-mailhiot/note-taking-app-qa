...
## Contents
- `notes-app-smoke.postman_collection.json` - the smoke collection (Auth, Notes, Negative folders)
...
- `POST /auth/login` (TC-004) - logs in as the registered user
- `POST /auth/logout` (TC-006) - logs out and clears the session cookie
...
Auth uses `express-session` with a `connect.sid` cookie. Postman's cookie jar picks up the `Set-Cookie` from login automatically and sends it with subsequent requests, so no manual header wiring is needed. The two self contained negative tests (TC-005 invalid login and TC-007 unauthenticated /notes) clear the jar in a pre-request script so each request hits the app as a logged-out user regardless of prior state.
